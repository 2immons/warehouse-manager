<template>
    ...
    <div class="section-content">
      <div class="documents-list">
        <div class="document-wrapper documents-list--header">
          <div class="document__title">Название</div>
          <div class="document__description">Описание</div>
          <div class="document__buttons">Управление</div>
        </div>
        <div class="document-wrapper" v-for="(item, index) in filteredItems" :key="index">
          <div class="document__title">{{ item.title }}</div>
          <div class="document__description">{{ item.description }}</div>
          <div class="document__buttons">
            <button class="buttons__btn table-button--edit show-document" @click="openUpdateDocumentForm(item)">
            ...
            <button class="buttons__btn table-button--remove show-document" @click="openSubmitForm(item)">
      ...
      <div class="documents-footer">
        <button class="buttons__btn add-document" @click="openCreateDocumentForm">Добавить документ</button>
      </div>
    </div>
</template>
...
import UpdateDocumentForm from '@/components/UpdateDocumentForm.vue'

export default {
  components: {
    CreateDocumentForm, SubmitForm, UpdateDocumentForm
  },
  ...
  mounted () {
    this.fetchDocuments().then(() => {
      this.filteredItems = this.getDocuments
    })
  ...
  methods: {
    ...
    async fetchDocumentsFromServer () {
      await this.fetchDocuments().then(() => {
        this.filteredItems = this.getDocuments
      })
    },
