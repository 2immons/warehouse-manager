<template>
    <header class="header">
      ...
      <button class="notifications-button" @click.stop @click="toggleNotifications">
        ...
        <div v-if="this.notificationsCount > 0" class="notifications-button__count">{{ notificationsCount }}</div>
      </button>
    ...
    <div v-if="isNotificationsVisible" @click="isNotificationsVisible = false" class="notifications-overlay">
        <div @click.stop class="notifications">
          ...
          <div class="notifications__items">
            <button @click="markNotificationAsRead">Пометить все как прочитанные</button>
            <p v-if="this.notifications.length === 0" class="notifications__error">В данный момент уведомлений нет</p>
            <div class="notifications__item" v-for="notification in notifications" :key="notification">
              ...
              <button @click="markNotificationAsRead2(item)">Прочитать</button>
            </div>
          </div>
        </div>
    </div>
  ...
  methods: {
    ...
    async fetchLogsFromServer () {
      try {
        ...
        this.notificationsCount = this.notifications.length > 9 ? '9+' : this.notifications.length
      } catch (error) {
