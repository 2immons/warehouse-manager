<template>
  <UpdateUserForm :currentUser="this.currentUser" v-if="isUpdateFormVisible" @update-user="fetchUsersFromServer" @close-popup="closeUpdateUserForm"/>
  <SubmitForm v-if="isSubmitFormVisible" @confirm="confirmActions" @deny="denyActions"/>
  <div class="section-header">
    <h2 class="section-header__text">ПОЛЬЗОВАТЕЛИ</h2>
  </div>
  <div class="section-content">
    <div class="users-list">
      <div class="user-wrapper user-wrapper--header">
        <div class="name">Имя</div>
        <div class="username">Логин</div>
        <div class="email">Почта</div>
        <div class="role">Роль</div>
        <div class="user__buttons">Управление</div>
      </div>
      <div class="user-wrapper" v-for="(item, index) in filteredItems" :key="index">
        <div class="name">{{ item.name }}</div>
        <div class="username">{{ item.username }}</div>
        <div class="email">{{ item.email }}</div>
        <div class="role">{{ item.role_name }}</div>
        <div class="user__buttons">
          <button class="buttons__btn table-button--edit show-document" @click="openUpdateUserForm(item)">
          ...
          <button class="buttons__btn table-button--remove show-document" @click="deleteUser(item)">
...
export default {
  ...
  mounted () {
    this.fetchUsers().then(() => {
      this.filteredItems = this.getUsers
    })
  },
  methods: {
    ...
    closeUpdateUserForm: function () { this.isUpdateFormVisible = false },

    deleteUser (item) {
      this.userForDelete = item
      this.userForDelete.is_deleted = true
      this.isSubmitFormVisible = true
    },

    async confirmActions () {
      try {
        await this.$store.dispatch('updateUser', this.userForDelete)

        const currentDateTime = new Date().toLocaleString('ru-RU', { timeZone: 'Europe/Samara' })

        await this.$store.dispatch('createLog', {
          user_id: Number(sessionStorage.getItem('userId')),
          operation: 'удален пользователь: ' + this.userForDelete.username,
